...
def Loc_Credible(ADP,Loc,Loc_database,ADP_database,Previous_Loc):
    if len(Previous_Loc) > 0 and  np.linalg.norm(Loc - Previous_Loc) > 2 :
        ...
        return out
    KNN = np.where((np.linalg.norm(Loc_database - Loc, axis = 1 ) < 1))
...
def Loc_ADP_Recovery_TF2D(ADP,Pred_ADP,Previous_Loc,Loc_database,ADP_database,model):
    ...
    KNN = np.asarray(KNN).T
    W = []
    if (np.linalg.norm(FP_Localizer_TF2D(Pred_ADP,model) - Previous_Loc) > 1):
        Pred_Loc = Previous_Loc
    else:
...
DCNNmodel = tf.keras.models.load_model('DCNNweights/Weights-589--0.11930.hdf5')
...
# Database
training_database_path ='data/DCNN-train.npz'
...
ADP_database = np.reshape(ADP_database,(199100,64,64))

# Test frames Load
data_path ='data/testframesO1.npz'
data = load(data_path)
...
def mytest(input, model):
    input = np.reshape(input,(1, 10, 64, 64, 1))
    input = np.concatenate((input,np.zeros((1, 10, 64, 64, 1))), axis = 1)
    avg_mse = 0
    batch_id = 0
    img_mse, ssim = [], []

    real_input_flag = np.zeros((1, 9, 16, 16, 16))
    out = np.zeros((1,10,64,64,1))
    test_ims = input[0:1]
...
Credible_Est_ADP = np.zeros((1000,20,64,64))
...
for i in range(0,1000):
        ...
        for j in range(0,20):
            ...
            if j > 0 and Loc_Credible(test_series1[j:j+1],Estimated_Loc_FP[j],Loc_database,ADP_database ,Credible_Est_Loc[i,j-1]) == 'Location is Credible':
                ...
                Credible_Est_ADP[i,j] = np.reshape(test_series[j:j+1],(64,64))
                if j > 9:
            ...
            if j == 0 and Loc_Credible(test_series1[j:j+1],Estimated_Loc_FP[j],Loc_database,ADP_database ,[]) == 'Location is not Credible':
                Credible_Est_ADP[i,j] = np.reshape(test_series1[j:j+1],(64,64))
                Credible_Est_Loc[i,j] = Estimated_Loc_FP[j]
                print('0 warning')
                if j < 10:
                    print('warning')
            if j > 0 and Loc_Credible(test_series1[j:j+1],Estimated_Loc_FP[j],Loc_database,ADP_database ,Credible_Est_Loc[i,j-1]) == 'Location is not Credible':    
                if j < 10:
                    print('warning')                   
                    Credible_Est_Loc[i,j], CredADP = Loc_ADP_Recovery_TF2D(test_series1[j],np.reshape(Credible_Est_ADP[i,j-1],(1,64*64)),Credible_Est_Loc[i,j-1],Loc_database,ADP_database,DCNNmodel)
                    Credible_Est_ADP[i,j] = np.reshape(CredADP,(64,64))
                else:
                    ...
                    if j == 19:
                        sys.exit()'''
                    Credible_Est_Loc[i,j], CredADP = Loc_ADP_Recovery_TF2D(test_series1[j],np.reshape(PredADP[0, 0, :, :, 0],(1,64*64)),Credible_Est_Loc[i,j-1],Loc_database,ADP_database,DCNNmodel)
                    Credible_Est_ADP[i,j] = np.reshape(CredADP,(64,64))
            if j > 9 :
                PredADP = mytest(Credible_Est_ADP[i,j-10:j], model) 
...
for i in range(0,1000):
        print(i)
        # Creating a test data
        test_series1 = np.reshape(CLADPNL_series[i,:,:,:],(20,64,64))
        test_series = test_series1
        test_series_Loc = Loc_series[i,:,:]
        Estimated_Loc_FP = np.zeros((20,3))
        PredADP = np.zeros((1, 10, 64, 64, 1))
...
for i in range(0,1000):
        print(i)
        # Creating a test data
        test_series1 = np.reshape(CLADPANL_series[i,:,:,:],(20,64,64))
        test_series = test_series1
        test_series_Loc = Loc_series[i,:,:]
        Estimated_Loc_FP = np.zeros((20,3))
        PredADP = np.zeros((1, 10, 64, 64, 1))

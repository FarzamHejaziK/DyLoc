...
def get_data(data,xnum,ynum):
    num_classes = xnum*ynum
    #get train data
    M = np.reshape(data['ADP'],(66550,32,32,1))
    loc = data['Loc']
    x = loc[:,0]
    y = loc[:,1]
...
model = Sequential()
# L = 1
model.add(Conv2D(8, (16,16), padding='same', kernel_initializer='normal',input_shape = (32,32,1), activation='relu'))
model.add(BatchNormalization())
model.add(MaxPool2D(pool_size=(2, 2)))

# The Hidden Layers :
model.add(Conv2D(16, (8,8), padding='same', kernel_initializer='normal', activation='relu'))
model.add(BatchNormalization())
model.add(MaxPool2D(pool_size=(2, 2)))
model.add(Conv2D(32, (7,7), padding='same', kernel_initializer='normal', activation='relu'))
model.add(BatchNormalization())
model.add(MaxPool2D(pool_size=(2, 2)))
model.add(Conv2D(64, (5,5), padding='same', kernel_initializer='normal', activation='relu'))
model.add(BatchNormalization())
model.add(MaxPool2D(pool_size=(2, 2)))
model.add(Conv2D(128, (3,3), padding='same', kernel_initializer='normal', activation='relu'))
model.add(BatchNormalization())
model.add(MaxPool2D(pool_size=(2, 2)))
